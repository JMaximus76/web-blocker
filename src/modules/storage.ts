...
import type {StorageInfo, StorageItemMap, InfoList, Settings, PromiseError, List, Mode, ListEntry, Timer } from '../modules/types';
...
// export async function updateInfo(info: StorageInfo): Promise<void> {
...
// export function compairInfo(x: StorageInfo, y: StorageInfo): boolean {
//     return (x.mode === y.mode) && (x.name === y.name);
// };

// export function isInInfoList(info: StorageInfo, infoList: InfoList): boolean {
//     return infoList[info.mode].some((entry: StorageInfo) => compairInfo(info, entry));
// }

// export function  getActiveInfos(infoList: InfoList): StorageInfo[] {
//     // const active: Info[] = [];
//     // infoList[infoList.activeMode].forEach((info: Info) => {
//     //     if (info.active) active.push(info);
//     // });
//     return infoList[infoList.activeMode].filter( (info: StorageInfo) => info.active);
// }

// export function getIndex(info: StorageInfo, infoList: InfoList): number {
//     return infoList[info.mode].findIndex((element) => compairInfo(info, element));
// }

// export function generateStorageKey(info: StorageInfo): string {
...
// export async function removeList(info: StorageInfo): Promise<void> {
//     const infoList = await getStorageItem("infoList");
//     const index = getIndex(info, infoList);
//     if (index === -1) {
//         const error: PromiseError = {
...
// export async function addListEntry(info: StorageInfo, entry: ListEntry): Promise<void> {
...
// export async function updateList(info: StorageInfo, list: List): Promise<void> {
//     const infoList = await getStorageItem("infoList");
...
// export async function modifyList(info: StorageInfo, listId?: string, mode?: Mode): Promise<void> {
//     const infoList = await getStorageItem("infoList");
...
// export async function registerNewList(info: StorageInfo): Promise<void> {
//     if (info.name === "") {
//         const error: PromiseError = {
...
// export async function getList(info: StorageInfo) {
...
//         const total = (timers[name].total * minute) + ((Date.now() - timers[name].start) % minute);
